      ...
      {shows.map((show, idx) => (
        ...
        >
          <div className="text-sm text-white mx-auto flex flex-col">
            <p className="text-sm font-semibold mx-auto leading-3 titulo">{show.fecha}</p>
            <p className="text-sm leading-5 titulo mt-[2px]">
              {show.lugar} – {show.ciudad} ({show.pais})
            </p>
          </div>

          {show.soldOut ? (
            <span className="bg-red-600 text-white text-base py-1 px-3 rounded font-bold absolute -right-3 top-1/2 transform -translate-y-1/2 -rotate-12 font-roboto">
              SOLD OUT
            </span>
          ) : (
            <a
